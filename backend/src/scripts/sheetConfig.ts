 ...
 */
async function validateSheetStructure(): Promise<void> {
...
/**
 * Clear data from specified sheets
 */
async function clearSheetData(sheetNames?: string[]): Promise<void> {
  const sheetsService = createGoogleSheetsService();
  if (!sheetsService) {
    throw new Error('Failed to initialize Google Sheets service');
  }

  const sheetsToClean = sheetNames || SHEET_CONFIGURATIONS.map(config => config.name);
 ...
 */
async function getSheetStatistics(): Promise<void> {
  const sheetsService = createGoogleSheetsService();
  if (!sheetsService) {
    throw new Error('Failed to initialize Google Sheets service');
  }
      ...
      try {
        ...
        if (recordCount > 0) {
          if (sheetConfig.name === 'Projects') {
            const activeProjects = records.filter((r: any) => r.status === 'active').length;
            stats[sheetConfig.name].active = activeProjects;
          } else if (sheetConfig.name === 'Invoices') {
            const paidInvoices = records.filter((r: any) => r.status === 'paid').length;
            const totalAmount = records.reduce((sum: number, r: any) => sum + (parseFloat(r.total_amount) || 0), 0);
            stats[sheetConfig.name].paid = paidInvoices;
            stats[sheetConfig.name].totalAmount = totalAmount;
          } else if (sheetConfig.name === 'Tasks') {
            const completedTasks = records.filter((r: any) => r.status === 'completed').length;
...
export { validateSheetStructure, clearSheetData, getSheetStatistics };