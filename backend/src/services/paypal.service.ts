...
export class PayPalPaymentGateway implements PaymentGateway {
  ...
  async processWebhook(payload: any, signature?: string): Promise<WebhookResult> {
    // PayPal webhook verification would typically involve verifying the webhook signature
  ...
  private async createOrder(params: CreatePaymentLinkParams & { successUrl?: string; cancelUrl?: string }): Promise<PayPalOrder> {
    const accessToken = await this.getAccessToken();
  ...
  private mapOrderToStatus(order: PayPalOrder): PaymentStatus {
