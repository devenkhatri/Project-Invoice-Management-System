...
export class GoogleSheetsService implements SheetsServiceInterface {
  private sheets: any;
   ...
   */
  private rowToObject(headers: string[], row: any[]): any {
    const obj: any = {};
   ...
   */
  private objectToRow(headers: string[], obj: any): any[] {
  ...
  private async findRowIndexById(sheetName: string, id: string): Promise<number> {
    ...
    return index >= 0 ? index + 2 : -1; // +2 because of header row and 0-based index
  }
  ...
  async read(sheetName: string, id?: string): Promise<any[]> {
    try {
      ...
      const data = rows.slice(1).map((row: any[]) => this.rowToObject(headers, row));

      if (id) {
        return data.filter((record: any) => record.id === id);
      }
  ...
  async delete(sheetName: string, id: string): Promise<boolean> {
    try {
      ...
      const sheet = spreadsheetInfo.sheets?.find((s: any) => s.properties.title === sheetName);
      if (!sheet) {
   ...
   */
  async batchUpdate(sheetName: string, updates: any[]): Promise<boolean> {
    try {
      const headers = await this.getHeaders(sheetName);
      const timestamp = new Date().toISOString();
  ...
  /**
   * Query records with filters (supports both array and object formats)
   */
  async query(sheetName: string, filters: QueryFilter[] | Record<string, any>): Promise<any[]> {
    try {
      const allData = await this.read(sheetName);
      
      // Handle object-style filters (for backward compatibility with auth service)
  ...
  /**
   * Aggregate data from a sheet
   */
  async aggregate(sheetName: string, operation: string, field: string): Promise<number> {
    try {
      const data = await this.read(sheetName);
  ...
  /**
   * Generic method to read data from a sheet (legacy method for backward compatibility)
   */
  ...
  /**
   * Generic method to append data to a sheet (legacy method for backward compatibility)
   */
  async appendToSheet(sheetName: string, data: any[]): Promise<boolean> {
    try {
      await this.sheets.spreadsheets.values.append({
        spreadsheetId: this.spreadsheetId,
        range: `${sheetName}!A:A`,
  ...
  /**
   * Initialize all required sheets for the project management system
   */
  async initializeProjectSheets(): Promise<boolean> {
    const sheetsConfig = [
      {
        name: 'Users',
